...
public class IngresoSQL {
	...
	public static final String FIND_LAST = "SELECT id_ingreso, cantidad_ingreso, puesto_ingreso, nivel_ingreso, lado_ingreso, fecha_ingreso, nombre_planta, nombre_empleado, apellido_empleado, nombre_bloque , numero_cama, fk_id_siembra FROM "+TABLE_NAME+", cama, empleado, planta, bloque, siembra WHERE id_bloque = fk_id_bloque and id_planta = fk_id_planta and id_cama = fk_id_cama and id_empleado = ingreso.fk_id_empleado and fk_id_siembra = id_siembra and activo_ingreso = 'Y' and fecha_ingreso = ( SELECT MIN(fecha_ingreso) FROM "+TABLE_NAME+" WHERE  fk_id_origen = ? AND fk_id_planta= ? )";
