app.controller("bloqueController",  ["$scope",  "bloqueService", function bloqueController($scope, bloqueService) {
	$scope.mostrarDatosBloque= {};
	$scope.id ="";	
	$scope.bloqueOut = {};
	$scope.bloqueSearch = function(id){		
		bloqueService.bloqueSearch(id).then(function (message) {	
			$scope.bloqueOut={"id":message.data.id,"nombre":message.data.nombre,
					"ncama":message.data.ncama,"finca":message.data.finca};
	...
	$scope.saveBloque=function(bloqueIn){
		bloque={"id":bloqueIn.id,"nombre":bloqueIn.nombre,
				"ncama":bloqueIn.ncama,"finca":bloqueIn.finca};
