app.controller("fincaController", [ "$scope", "fincaService",
		function fincaController($scope, fincaService) {
			...
			$scope.fincaSearch = function(id) {
				fincaService.fincaSearch(id).then(function(message) {
					$scope.fincaOut = {
						...
						"ubicacion" : message.data.ubicacion,
			...
			$scope.saveFinca = function(fincaIn) {
				finca = {
					"id" : fincaIn.id,
					"nombre" : fincaIn.nombre,
					"ubicacion" : fincaIn.ubicacion,
					"telefono1" : fincaIn.telefono1
				};
				$scope.showForm = false;
