app.controller("productoController",  ["$scope",  "productoService", function productoController($scope, productoService) {
	$scope.mostrarDatosProducto= {};
	$scope.id ="";	
	$scope.productoOut = {};
	$scope.productoSearch = function(id){		
		productoService.productoSearch(id).then(function (message) {	
			$scope.productoOut={"id":message.data.id,"nombre":message.data.nombre};
	...
	$scope.saveProducto=function(personaIn){
		producto={"id":productoIn.id,"nombre":productoIn.nombre};
		productoService.saveProducto(productoIn).then(function(message){
			alert(message.data);
		})
	}
	$scope.productoIn={};
	$scope.updateProducto= function(productoIn){
		producto={"id":productoIn.id,"nombre":productoIn.nombre};
		productoService.updateProducto(productoIn).then(function(message){
			alert(message.data);
		});
	}
}]);