app.controller("empleadoController", [
		"$scope",
		"empleadoService",
		function empleadoController($scope, empleadoService) {
			...
			$scope.empleadoSearch = function(id) {
				empleadoService.empleadoSearch(id).then(function(message) {
					$scope.empleadoOut = {
						"id" : message.data.id,
						"tipodoc" : message.data.tipodoc,
						"documento" : message.data.documento,
			...
			$scope.saveEmpleado = function(empleadoIn) {
				empleado = {
					"id" : empleadoIn.id,
					"tipodoc" : empleadoIn.tipodoc,
					"documento" : empleadoIn.documento,
					"nombre" : empleadoIn.nombre,
					"apellido" : empleadoIn.apellido
				};
				$scope.showForm = false;
				empleadoService.saveEmpleado(empleadoIn).then(
						function(message) {
							alert(message.data);
						})
				$scope.empleadoIn = {};
			...
			$scope.updateEmpleado = function(empleadoIn) {
				...
				empleadoService.updateEmpleado(empleadoIn).then(
						function(message) {
							alert(message.data);
						});
			}
